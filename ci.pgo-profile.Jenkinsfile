  ...
  buildmatrix[name] = {
    node('docker && !magrathea') {
      helpers.build([
        ...
        objDir: objDir,
        artifactGlob: "$objDir/dist/Umbra-*",
        locales: [],
        buildId: buildId,
        Reset: true,
        Instrument: true,
        ...
        stage('run profileserver') {
          sh "BINARY=${objDir}/dist/Umbra/Umbra bash ${env.WORKSPACE}/ci/linux_profileserver.sh"
...
if (params.MacOSX64) {
  ...
  buildmatrix[name] = {
    node('docker && !magrathea') {
      ...
      stash name: name, includes: "mozilla-release/${objDir}/dist/Umbra-*.dmg"
    }
      ...
      stage('run profileserver') {
        ...
        dir('mozilla-release') {
          sh '''#!/bin/bash
            ...
            ./mach python python/mozbuild/mozbuild/action/install.py ./obj-x86_64-apple-darwin/dist/Umbra-*.dmg /tmp/
            ./mach python build/pgo/profileserver.py --binary /tmp/*.app/Contents/MacOS/Umbra
          '''
          sh "mkdir -p $WORKSPACE/${name}/"
          sh "tar -Jcvf $WORKSPACE/${name}/profdata.tar.xz merged.profdata en-US.log"
        }
