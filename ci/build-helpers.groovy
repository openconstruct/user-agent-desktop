            ...
            withEnv(defaultEnv) {
                dir('mozilla-release') {
                    ...
                    stage("${opts.name}: generate MAR") {
                        ...
                        sh """
                            ./mach repackage mar \
                                -i ${opts.objDir}/dist/Umbra-2022.2.1.en-US.mac.tar.gz \
...
def prepare_workspace(reset, targetPlatform, skipPatches) {
    ...
    sh "./fern.js config --print --force -l --platform ${targetPlatform} --brand Umbra"
...
def windows_signed_packaging(name, objDir, appName='Umbra') {
...
def mac_unified_dmg() {
    ...
    image.inside() {
        ...
        withEnv(["MOZCONFIG=${env.WORKSPACE}/mozconfig"]) {
            ...
            archiveArtifacts artifacts: "mozilla-release/${x86ObjDir}/dist/Umbra-*"
            // restash MacOSX64 artifacts
            stash name: 'MacOSX64', includes: [
                "mozilla-release/${x86ObjDir}/dist/Umbra-*",
...
def withGithubRelease(Closure body) {
    docker.image('golang').inside("-u root") {
        sh 'go get github.com/umbra/github-release'
        withCredentials([
            usernamePassword(
