  ...
  return new Listr([
    {
      title: "init",
      task: () => {
        console.log(`DEBUG: Initializing git in folder: ${folder}`);
        return withCwd(folder, () => execa("git", ["init"]));
      },
    },
    {
      title: "identity",
    ...
    {
      title: "add",
      task: () => withCwd(folder, () => execa("git", ["add", "-A"])),
    },
    {
      title: "commit",
      task: () =>
...
async function importFromPatchFiles(patchesFolder) {
  ...
  // Recover automatically from a previous interrupted `git am`.
  if (await folderExists(path.join(".git", "rebase-apply"))) {
    await abortPendingGitOperations();
  }

  if ((await fileExists(patchesIndex)) === false) {
    console.warn(`Skipping patch import for ${patchesFolder}: no .index file.`);
    return;
  }

  const patchEntries = (await fs.promises.readFile(patchesIndex, "utf-8"))
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith("#"));

  const missingPatches = [];
  const alreadyAppliedPatches = [];
  const patches = [];
  for (const filename of patchEntries) {
    ...
    if (await fileExists(patchPath)) {
      ...
      missingPatches.push(filename);
    }
  }

  if (missingPatches.length > 0) {
    console.warn(
      `Skipping missing patch files: ${missingPatches.join(", ")}`
    );
  }

  if (alreadyAppliedPatches.length > 0) {
    console.warn(
      `Skipping already-applied patches: ${alreadyAppliedPatches.join(", ")}`
    );
  }

  if (patches.length === 0) {
    console.warn("No patch files to import.");
    return;
  }
