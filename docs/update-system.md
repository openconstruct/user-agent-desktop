...
server to provide updates. We have a [fork](https://github.com/umbra/balrog) of that project
that contains the changes needed to configure the update server for our purposes, and the deployment
steps. The deployment is documented [here](https://github.com/umbra/balrog/blob/umbra/Deployment.md).

The balrog public endpoint is available at `update.umbrabrowser.com`, and we configure the browser
...
For Umbra we distribute the same builds to both channels in order to simplify the build process,
